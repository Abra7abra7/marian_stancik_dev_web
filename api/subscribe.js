// Vercel serverless — Lead & Order capture → AgentMail (branded HTML email templates + admin alerts)
...
export default async function handler(req, res) {
  try {
    ...
    if (isContact) {
      ...
      const userHtml = emailTpl.contactConfirmation(name, message);
      const userText = emailTpl.contactConfirmationText(name, message);

      await sendEmail(email, userSubject, userText, userHtml).catch(e => console.error('Contact user email failed:', e.message));
    ...
    const userHtml = emailTpl.welcome(name, source);
    const userText = emailTpl.welcomeText();

    await sendEmail(email, userSubject, userText, userHtml).catch(e => console.error('Newsletter user email failed:', e.message));
